...
local span = require("opentelemetry.api.trace.span"):new()
local span_context = require("opentelemetry.api.trace.span_context")
local span_kind = require("opentelemetry.api.trace.span_kind"):new()
local span_status = require("opentelemetry.api.trace.span_status"):new()
...
-- @param[type=table,opt={}] _attributes Table of attributes.
...
------------------------------------------------------------------------------------------------------------------------
-- A convenience method for wrapping code in a span (i.e. this method takes care of both starting and
-- finishing a span). It wraps the supplied function in a pcall; if there's an error, that gets recorded on the span and
-- re-raised.
--
-- Use it like so: tracer:in_span("my span name", function() print("hi") end)
--
-- @string name The name of the span
-- @param[type=function] fn A function to execute in the span (can be anonymous)
-- @param[type=table,opt={}] attributes Table of attributes
-- @param[type=table,opt={}] links Integer-keyed table of @{api.trace.link} instances
--
-- @return nil
------------------------------------------------------------------------------------------------------------------------
function _M:in_span(name, fn, attributes, links)
    local span = self:start_span(name, context.current(), span_kind.INTERNAL, attributes or {}, links or {})
