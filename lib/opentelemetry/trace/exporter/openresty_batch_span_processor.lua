...
--          opts.export_timeout_ms: maximum duration for exporting a batch, default 30000ms
...
function _M.on_end(self, span)
    ...
    if #self.queue >= self.max_queue_size then
        ...
        ngx.log(ngx.WARN, "Dropped span due to full queue: trace_id = ", dropped.ctx.trace_id, " span_id = ",
            dropped.ctx.span_id)
    end
